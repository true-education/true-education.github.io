...
스팩이 공모자금을 예치하는 상품의 고시금리를 받아 data/ksfc_rate.json 에
기준일자별로 남긴다. 신규 상장 종목을 추가할 때 1~3년차 예치이율로 쓸
값을 얻는 게 목적이라, 정기 실행이 아니라 그때그때 손으로 돌린다.
쌓인 스냅샷은 "그 종목에 이 값을 쓴 근거가 며칠자 고시였는지" 기록이 된다.
